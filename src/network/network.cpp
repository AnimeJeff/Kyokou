...
NetworkClient::Response NetworkClient::post(const QString &url, const QMap<QString, QString> &headers, const QMap<QString, QString> &data){
    ...
    return request(POST, url, headers, postData.toStdString ());
}

NetworkClient::Response NetworkClient::request(int type, const QString &url, const QMap<QString, QString> &headersMap, const std::string &postData){
    CURL *curl = getCurl ();
    if (!curl) {
    ...
    // Check for errors
    if (res != CURLE_OK){
        ErrorHandler::instance().show (QString("curl_easy_perform() failed: ") + curl_easy_strerror(res));
    }
