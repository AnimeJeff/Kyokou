...
#include "parsers/data/mediatype.h"

class Nivod: public MediaProvider
{
public:
    Nivod(){};
    ...
    QVector<MediaData> search(QString query, int page, int type) override{
        if(query == ""){
    ...
    QVector<MediaData> search(std::map<std::string, std::string> data){
        lastSearch=data;
    ...
    QVector<MediaData> filterSearch(int page, std::string sortBy,std::string channel,std::string regionId = "0",std::string langId="0",std::string yearRange=" ") {
        m_currentPage=page;
        std::map<std::string, std::string> data = {
            {"sort_by", sortBy},
            {"channel_id", channel},
    ...
    QVector<MediaData> filterSearch(std::map<std::string, std::string> data) {
        lastSearch=data;
        filterSearched=true;
    ...
    QVector<MediaData> popular(int page, int type)override {
        return filterSearch(page,"1",channelId[static_cast<MediaType>(type)]);
    }

    QVector<MediaData> latest(int page, int type)override {
        return filterSearch(page,"4",channelId[static_cast<MediaType>(type)]);
    }

    MediaData loadDetails(MediaData show) override{
        ...
        if(!entity["hot"].is_null ()){
            show.views = QString::number(entity["hot"].get<int>());
        }
...
private:
    ...
    const QMap<MediaType, std::string> channelId = {
        {MediaType::Movie, "1"},
        {MediaType::TvSeries, "2"},
        {MediaType::Reality, "3"},
        {MediaType::Anime, "4"},
        {MediaType::Documentary, "6"}
    };
private:
...
private:
    QVector<MediaData> showsFromJsonArray(const nlohmann::json& showList,int type = 0) {
        QVector<MediaData> results;
        for (auto& el : showList.items())
        {
            auto item=el.value ();
            MediaData show;

            MediaType tvType = MediaType::Anime;
            std::string channelName = item["channelName"].get<std::string>();
            if ( channelName== "电影") {
                tvType = MediaType::Movie;
            } else if (channelName == "电视剧") {
                tvType = MediaType::TvSeries;
            } else if (channelName == "综艺") {
                tvType = MediaType::Reality;
            } else if (channelName == "动漫") {
                tvType = MediaType::Anime;
            } else if (channelName == "纪录片") {
                tvType = MediaType::Documentary;
            }
            ...
            //            if (MediaData.latestTxt.contains("更新")) {
