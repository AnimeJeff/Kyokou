...
QList<Video> PlaylistModel::loadOnlineSource(int playlistIndex, int itemIndex)
{
    ...
    qInfo()<<"Log (Playlist): Fetching servers for episode" << episodeName;
    qInfo()<<"Log (Playlist): Playlist index:" << itemIndex + 1 << "/" << playlist->count ();
...
void PlaylistModel::play(int playlistIndex, int itemIndex = -1)
{
    ...
    case PlaylistItem::LOCAL: {
        ...
        m_watcher.setFuture(QtConcurrent::run([playlist, itemIndex]() {
            return QList<Video> {Video(playlist->loadLocalSource(itemIndex)) };
        }));
        break;
    }
    ...
    m_watcher.future ()
        .then([this, itemIndex, playlist](QList<Video> videos)
              {
                  if (!videos.isEmpty ()) {
                  ...
                  setLoading(false);
                  if (videos.isEmpty ()) ErrorHandler::instance ().show ("Failed to get playable source for this episode");
              })
...
void PlaylistModel::loadOffset(int offset)
{
    ...
    // if(offset == 1 && nextVideoSource.isValid ())
    // {
