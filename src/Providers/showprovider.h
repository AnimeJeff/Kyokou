...
public:
    ...
    virtual QList<ShowData> reload() const final { return lastSearch(m_currentPage); }

    virtual bool canFetchMore() const final { return m_canFetchMore; }
    virtual QList<ShowData> fetchMore() const final { return lastSearch(m_currentPage + 1); }
