...
QList<ShowData> Nivod::filterSearch(int page, const QString &sortBy, int type,
  ...
  std::string response =
      callAPI("https://api.nivodz.com/show/filter/WEB/3.2", data);
  QList<ShowData> results =
      showsFromJsonArray(nlohmann::json::parse(response)["list"]);
...
std::string
Nivod::callAPI(const std::string &url,
...
QList<VideoServer> Nivod::loadServers(const PlaylistItem *episode) const {
  return QList<VideoServer>{VideoServer{
      "default", episode->link, {{"referer", "https://www.nivod.tv/"}}}};
}

QMap<QString, QString>
Nivod::objKeySort(const QMap<QString, QString> &inputMap) const {
  if (inputMap.isEmpty())
    return inputMap;
...
QList<ShowData> Nivod::showsFromJsonArray(const nlohmann::json &showList,
  ...
  for (auto &el : showList.items()) {
    ...
    QString title =
        QString::fromStdString(item["showTitle"].get<std::string>());
    QString coverUrl =
        QString::fromStdString(item["showImg"].get<std::string>());
    std::string link = item["showIdCode"].get<std::string>();

    QString latestTxt = "";
    if (!item["episodesTxt"].is_null()) {
      latestTxt =
          QString::fromStdString(item["episodesTxt"].get<std::string>());
